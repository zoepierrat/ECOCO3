...
def plot_coincident_site_map(df_coincident, metadata, output_path=None):
    """World map of the FLUXNET tower sites used in the coincident
    FLUXNET/ECOCO3 comparison (plot_flux_vs_eco), one color per site — the
    same site -> color mapping used by plot_flux_vs_eco_by_site, so this
    map doubles as that plot's color key.

    Returns (sites, site_colors): the tagged site table (with its color)
    and the site -> color dict, so the mapping can be reused or the sites
    displayed as a lookup table.
    """
    sites = attach_site_id(df_coincident[['site_lat', 'site_lon']].drop_duplicates(), metadata)
    site_colors = assign_site_colors(sites['Site'].dropna())
    colors = [site_colors.get(s, 'lightgray') for s in sites['Site']]
    ...
    ax.axhline(51.6, color='blue', linestyle='--', linewidth=1)
    ax.axhline(-51.6, color='blue', linestyle='--', linewidth=1)

    ax.scatter(sites['site_lon'], sites['site_lat'], s=120, c=colors,
               edgecolor='black', linewidth=0.8, zorder=5, transform=ccrs.PlateCarree())

    ax.set_title(f'FLUXNET Sites in the ECOCO3 Validation Comparison (n={len(sites)})',
                 fontsize=22)
    ...
    return sites.assign(color=colors), site_colors


def plot_flux_vs_eco_by_site(df_hh, df_daily=None, metadata=None, output_path=None):
    """Same comparison as plot_flux_vs_eco, but points are colored by
    FLUXNET site instead of hour-of-day/month. With ~50 sites a legend
    isn't readable inline — cross-reference colors against
    plot_coincident_site_map(), which uses the same site -> color mapping.
    """
    datasets = [('Half-hourly', df_hh, 'hh')]
    if df_daily is not None and len(df_daily) > 0:
        datasets.append(('Daily', df_daily, 'daily'))
    ...
    for row, (label, _, res) in enumerate(datasets):
        ...
        for i, var in enumerate(variables):
            ...
            lim = [min(x.min(), y.min()), max(x.max(), y.max())]
            ax.plot(lim, lim, '--', lw=2, color='black')
    ...
    fig.suptitle('Colored by site — see plot_coincident_site_map() for the site/color key',
                  fontsize=15, y=1.01)
    plt.tight_layout()
    if output_path:
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def hour_to_color(h, h_peak=12, sigma=4.0):
    import colorsys
    hue = h / 24.0
    brightness = np.exp(-0.5 * ((h - h_peak) / sigma) ** 2)
